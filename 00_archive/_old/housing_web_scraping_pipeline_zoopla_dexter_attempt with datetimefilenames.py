...
def web_scraping_function_zoopla(**kwargs):
    ...
    #name of the file
    key = Variable.get("housing_webscraping_get_csv", deserialize_json=True)['key2']+date_time+".csv"
...
def web_scraping_function_londonair(**kwargs):
        ...
        bucket_name = kwargs['bucket_name']
        #name of the file
        key = Variable.get("housing_webscraping_get_csv", deserialize_json=True)['key4']
...
def save_result_to_postgres_db_dexter(**kwargs):
    ...
    log.info('passing Dexter data from S3 bucket')
    ...
    log.info('Finished saving the Dexter data to postgres database')
